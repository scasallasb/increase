    ...
    while queue:
        ...
        for neighbor, w in G[curnode].items():
            ...
            if neighbor in enqueued:
                ...
                h = (distNodeTarget(G,curnode, listNodeTarget)*heuristic(G,neighbor))/maximaDist
                costo.append(h)
...
def distNodeTarget(G,node1, listTarget):
    aux = 999
    for i in listTarget:
        dis = distancia(G,node1, i )
        if dis < aux:
            aux = dis
    return aux
...
def single_source_dijkstra(G, source, target=None, cutoff=None, weight='weight', N="N"):
    ...
    def func(u,v, d):
        ...
        if u in N :
            return 1
        node_v_wt = G.nodes[v].get(weight, 1)+ distancia(G,u, v)
        return node_v_wt
