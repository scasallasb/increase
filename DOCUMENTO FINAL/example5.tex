...
y toma como heurística \((l/d)*Cmin\), donde cada variable es calculada
por una función:

\(l\) es la función
\texttt{distMinNodeTarget(G,curnode,\ listNodeTarget)} que tiene como
parámetro de entrada el grafo \(G\), el nodo actual \(curnode\) y una
lista de nodos objetivos \(listNodeTarget\). La salida es la distancia
mínima del nodo que se está analizando y cualquiera de los nodos
objetivo. En esta función se obtiene del parametro \(G\) el valor de la
posición de los nodos \emph{pos} que contiene las coordenadas de latitud
y longitud, luego, mediante la función \texttt{distancia} que retorna la
distancia en km de los enlaces para posteriormente comparar el valor de
las distancias y retornar la que tiene el mínimo valor. ``` def
distNodeTarget(nodo, listTarget):
...
\(d\) es la función \texttt{maximaDistancia(G)} que tiene como parámetro
\(G\) y retorna la distancia máxima en km de los enlaces.
